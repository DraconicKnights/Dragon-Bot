                    ...
                    MySql.query(
                        `SELECT * FROM GuildTicket WHERE guildId = '${interaction.guild.id}'`, (err, data) => {
                            ...
                            if(!data.guildId) {
                                ...
                                MySql.query(
                                    `DELETE FROM GuildTicket WHERE guildId = '${interaction.guild.id}'`,(err, row) => {
                                      if (err) {
                                          console.log(err);
                                      }
                                    })
                                ...
                                MySql.query(
                                    `INSERT INTO GuildTicket VALUES('${interaction.guild.id}', '${channel}')`
                                )
                            }



                            interaction.reply({content: `Ticket channel has been set succsesfully `, ephemeral: true});
                        })
                    break;
            }
        } 
